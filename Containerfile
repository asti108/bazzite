...
ARG LINUX_CEC_REF
RUN dnf5 -y install cargo dbus-devel git make pkgconf-pkg-config rust systemd-devel systemd-rpm-macros && \
...
RUN dnf5 -y install git make pkgconf-pkg-config && \
...
# FIXED: Replaced -printf '/%P ' with -printf '/%P\0' so xargs -0 matches correctly
...
# Ensure directory exists or handles edge cases cleanly before trying to apply patches
RUN if [ -d /tmp/kernel-build/linux-* ]; then \
...
RUN --mount=type=cache,dst=/var/cache \
    ...
    for copr in ublue-os/bazzite ublue-os/bazzite-multilib ublue-os/staging ublue-os/packages ycollet/audinux che/nerd-fonts; do \
...
# Package swapping syntax & correctly escape variables inside loop
RUN --mount=type=cache,dst=/var/cache \
    --mount=type=cache,dst=/var/cache/libdnf5 \
    --mount=type=cache,dst=/var/log \
    --mount=type=bind,from=ctx,source=/,target=/ctx \
    --mount=type=tmpfs,dst=/tmp \
    dnf5 -y remove pipewire-config-raop mesa-va-drivers && \
    dnf5 -y swap --from-repo=copr:copr.fedorainfracloud.org:ublue-os:bazzite wireplumber wireplumber && \
    dnf5 -y swap --from-repo=copr:copr.fedorainfracloud.org:ublue-os:bazzite-multilib bluez bluez && \
    dnf5 -y swap --from-repo=copr:copr.fedorainfracloud.org:ublue-os:bazzite-multilib xorg-x11-server-Xwayland xorg-x11-server-Xwayland && \
    dnf5 -y swap --from-repo=terra-mesa mesa-filesystem mesa-filesystem && \
    dnf5 -y swap --from-repo=copr:copr.fedorainfracloud.org:ublue-os:staging fwupd fwupd && \
    dnf5 versionlock add wireplumber wireplumber-libs bluez bluez-cups bluez-libs bluez-obexd xorg-x11-server-Xwayland mesa-dri-drivers mesa-filesystem mesa-libEGL mesa-libGL mesa-libgbm mesa-vulkan-drivers fwupd fwupd-plugin-flashrom fwupd-plugin-modem-manager fwupd-plugin-uefi-capsule-data NetworkManager NetworkManager-wifi NetworkManager-libnm && \
    dnf5 --enable-repo=terra-mesa -y install mesa-libOpenCL intel-opencl clinfo && \
    dnf5 -y install libfreeaptx && \
    dnf5 -y install --enable-repo="*fedora-multimedia*" libbluray libbluray-utils makemkv && \
...
RUN --mount=type=cache,dst=/var/cache \
    ...
    --mount=type=bind,from=ctx,source=/,target=/ctx \
    --mount=type=tmpfs,dst=/tmp \
    dnf5 -y remove ublue-os-update-services firefox firefox-langpacks htop && \
...
RUN --mount=type=cache,dst=/var/cache \
    ...
    dnf5 -y install $(/ctx/ghcurl https://api.github.com/repos/ublue-os/cicpoffs/releases/latest | jq -r --arg name "cicpoffs-fc${FEDORA_VERSION}.rpm" '.assets[] | select(.name == $name).browser_download_url') && \
    dnf5 -y copr enable bieszczaders/kernel-cachyos-addons && \
    dnf5 -y install scx-scheds scx-tools && \
    dnf5 -y copr disable bieszczaders/kernel-cachyos-addons && \
    dnf5 -y install fuse-libs uld bazaar iwd greenboot greenboot-default-health-checks bazzite_updater ScopeBuddy twitter-twemoji-fonts google-noto-sans-cjk-fonts lato-fonts fira-code-fonts nerd-fonts python3-pip libadwaita bees xwininfo usbip compsize ryzenadj ddcutil input-remapper libinput-utils i2c-tools lm_sensors iio-sensor-proxy fw-ectool fw-fanctrl framework-system udica ladspa-caps-plugins ladspa-noise-suppression-for-voice pipewire-module-filter-chain-sofa python3-icoextract tailscale webapp-manager btop amdsmi duf fish lshw xdotool wmctrl libcec linuxconsoletools vulkan-tools vulkan-low-latency-layer xwiimote-ng fastfetch glow gum vim cockpit-networkmanager cockpit-podman cockpit-selinux cockpit-system cockpit-files cockpit-storaged topgrade ydotool stress-ng snapper btrfs-assistant edk2-ovmf lsb_release uupd ds-inhibit waydroid cage wlr-randr gmodpatchtool bazzite-portal ls-iommu v4l-utils yad f3 lzip p7zip p7zip-plugins rar libxcrypt-compat && \
...
RUN --mount=type=cache,dst=/var/cache \
    ...
    if grep -q "kinoite" <<< "${BASE_IMAGE_NAME}"; then \
        dnf5 -y install plasma-foreground-booster-dmemcg ; \
    else \
        dnf5 -y swap --repo terra-extras uresourced uresourced-dmemcg ; \
    fi && \
    dnf5 --enable-repo=terra-mesa --enable-repo=terra -y install terra-gamescope.x86_64 terra-gamescope-libs.x86_64 terra-gamescope-libs.i686 jupiter-sd-mounting-btrfs umu-wrapper umu-launcher dbus-x11 xrandr evtest xdg-user-dirs xdg-terminal-exec gobject-introspection libFAudio.x86_64 libFAudio.i686 vkBasalt.x86_64 vkBasalt.i686 mangohud.x86_64 mangohud.i686 obs-studio-plugin-vkcapture-hook-libs.x86_64 obs-studio-plugin-vkcapture-hook-libs.i686 openxr && \
    dnf5 -y --enable-repo=terra-mesa --enable-repo=terra --setopt=install_weak_deps=False install steam lutris && \
...
RUN --mount=type=cache,dst=/var/cache \
    ...
    --mount=type=secret,id=GITHUB_TOKEN \
    if grep -q "kinoite" <<< "${BASE_IMAGE_NAME}"; then \
        dnf5 -y install qt krdp steamdeck-kde-presets-desktop kdeconnectd kdeplasma-addons rom-properties-kf6 fcitx5-chewing fcitx5-mozc fcitx5-chinese-addons fcitx5-hangul fcitx5-m17n kcm-fcitx5 gnome-disk-utility kio-extras krunner-bazaar krdc tesseract-devel tesseract-langpack-eng tesseract-langpack-spa tesseract-langpack-deu tesseract-langpack-jpn tesseract-langpack-jpn_vert tesseract-langpack-fra tesseract-langpack-por tesseract-langpack-rus tesseract-langpack-ita tesseract-langpack-nld tesseract-langpack-pol tesseract-langpack-tur tesseract-langpack-chi_sim tesseract-langpack-chi_sim_vert tesseract-langpack-chi_tra tesseract-langpack-chi_tra_vert tesseract-langpack-ces tesseract-langpack-ell && \
        dnf5 -y remove plasma-drkonqi plasma-welcome plasma-welcome-fedora plasma-discover-kns kcharselect kde-partitionmanager plasma-discover && \
        ...
        dnf5 -y install nautilus-gsconnect steamdeck-backgrounds steamdeck-gnome-presets gnome-shell-extension-user-theme gnome-shell-extension-gsconnect rom-properties-gtk4 rom-properties-localsearch3 ibus-mozc openssh-askpass firewall-config && \
        dnf5 -y remove gnome-software gnome-classic-session gnome-tour gnome-extensions-app gnome-system-monitor gnome-initial-setup gnome-shell-extension-background-logo gnome-shell-extension-apps-menu gnome-shell-extension-launch-new-instance gnome-shell-extension-places-menu gnome-shell-extension-window-list && \
...
RUN --mount=type=cache,dst=/var/cache \
    ...
    sed -i '/#UserspaceHID.*/UserspaceHID=true/' /etc/bluetooth/input.conf && \
    ...
    for repo in fedora-cisco-openh264 tailscale _copr_ublue-os-akmods terra terra-extras negativo17-fedora-multimedia; do \
        sed -i 's@enabled=1@enabled=0@g' /etc/yum.repos.d/$repo.repo; \
    done && for copr in ublue-os/bazzite ublue-os/bazzite-multilib ublue-os/staging ublue-os/packages ycollet/audinux che/nerd-fonts; do \
...
# 2. Open WebUI Service Definition
...
RUN --mount=type=cache,dst=/var/cache \
    ...
    --mount=type=tmpfs,dst=/tmp \
    dnf5 -y install sddm && \
    dnf5 -y remove jupiter-sd-mounting-btrfs ds-inhibit plasma-login-manager && \
    if grep -q "kinoite" <<< "${BASE_IMAGE_NAME}"; then \
        dnf5 -y remove steamdeck-kde-presets-desktop && dnf5 -y install steamdeck-kde-presets ; \
    else \
        ln -sf /usr/share/wallpapers/convergence.jxl /usr/share/backgrounds/default.jxl && \
        ln -sf /usr/share/wallpapers/convergence.jxl /usr/share/backgrounds/default-dark.jxl && \
        rm -f /usr/share/backgrounds/default.xml && \
        dnf5 -y remove malcontent-control ; \
    fi && \
    if grep -q "silverblue" <<< "${BASE_IMAGE_NAME}"; then \
...
RUN --mount=type=cache,dst=/var/cache \
    ...
    --mount=type=bind,from=ctx,source=/,target=/ctx \
    --mount=type=tmpfs,dst=/tmp \
    dnf5 -y install --enable-repo=terra jupiter-fan-control jupiter-hw-support-btrfs galileo-mura steamdeck-dsp powerbuttond inputplumber gamescope-session-ogui-steam steamos-manager-powerstation steamos-manager-powerstation-gamescope-session-plus vpower steam-notif-daemon acpica-tools sdgyrodsu ibus-pinyin ibus-table-chinese-cangjie ibus-table-chinese-quick socat zstd zenity qt6-qtvirtualkeyboard xorg-x11-server-Xvfb python-vdf python-crcmod && \
    ...
    git clone https://github.com/bazzite-org/jupiter-dock-updater-bin.git --depth 1 /tmp/jupiter-dock-updater-bin && \
    mv -v /tmp/jupiter-dock-updater-bin/package /usr/share/jupiter-dock-updater-bin && \
